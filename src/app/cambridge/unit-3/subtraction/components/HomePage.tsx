// src/app/cambridge/unit-3/subtraction/components/HomePage.tsx
"use client";
...
import { Home, Minus, BookOpen, Calculator, ArrowLeft } from "lucide-react";
...
export const HomePage = ({ setPage, language, onLanguageChange }: HomePageProps) => {
  ...
  const menuItems = [
    {
      id: "lesson" as const,
      title: language === "en" ? "📚 Learn Subtraction" : "📚 Học Phép Trừ",
      description: language === "en" 
        ? "Step-by-step lesson on 3-digit subtraction with borrowing using the Number House method"
        : "Bài học từng bước về phép trừ 3 chữ số có nhớ với phương pháp Ngôi nhà số",
      icon: BookOpen,
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-red-50 via-orange-50 to-yellow-50 relative overflow-hidden">
      ...
      <div className="absolute inset-0 opacity-10">
        ...
        />
        <motion.div
          className="absolute top-40 right-32 w-24 h-24 bg-orange-400 rounded-full blur-xl"
          animate={{
            x: [0, -80, 0],
            y: [0, 60, 0],
          }}
      ...
      <div className="container mx-auto px-4 py-8 relative z-10">
        ...
        >
          ...
          >
            <Minus className="w-24 h-24 text-red-600 mx-auto mb-4" />
          </motion.div>
          
          <motion.h1 
            className="text-6xl md:text-7xl font-bold bg-gradient-to-r from-red-600 via-orange-600 to-pink-600 bg-clip-text text-transparent mb-6"
            animate={{ 
        ...
        >
          <div className="bg-white/60 backdrop-blur-sm rounded-2xl p-8 max-w-4xl mx-auto border border-gray-200/50">
            ...
            <p className="text-gray-600 leading-relaxed">
              {language === "en" 
                ? "This method uses visual representation with a 'Number House' to help students understand the concept of borrowing in subtraction, making it intuitive and memorable."
                : "Phương pháp này sử dụng hình ảnh trực quan với 'Ngôi nhà số' để giúp học sinh hiểu bản chất của việc trừ có nhớ, làm cho nó trở nên trực quan và dễ nhớ."}
            </p>
          </div>
        </motion.footer>
      </div>
